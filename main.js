...
import { Cluster } from "patchright-browserpool-cluster";
...
const argv = yargs(hideBin(process.argv))
    .option("max_concurrency", {
        alias: "c",
        type: "number",
        describe: "Maximum number of concurrent workers",
        default: 30,
    })
    ...
    .option("sqlite_timeout", {
        type: "number",
        describe: "SQLite query timeout in milliseconds",
        default: 30000,
    })
    .option("proxy_domain", {
        type: "string",
...
const db = new Database('captcha_results.db', {timeout: argv.sqlite_timeout});
...
app.listen(argv.port, function () {
    db.exec(`
        CREATE TABLE IF NOT EXISTS turnstile_results (
...
async function queueTurnstile(req, res) {
    ...
    const insert = db.prepare(`
        INSERT INTO turnstile_results (
            id, status, created_at, proxy, cf_clearance,
            finished_at, user_agent, token
        ) VALUES (
            @id, @status, @created_at, @proxy, @cf_clearance,
            @finished_at, @user_agent, @token
        )
...
async function result(req, res) {
    ...
    const result = db.prepare('SELECT * FROM turnstile_results WHERE id = ?').get(id);
    if (result === undefined) {
        await res.status(400).json({
            msg:`"id ${id} not found`
        });
...
async function processTurnstile({page, data, worker}) {
    const HTML_TEMPLATE = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    const proxy = server.replace('://', `://${username}:${password}@`);
...
function resolve(result) {
    db.prepare(`
        UPDATE turnstile_results
        SET status = @status, finished_at = @finished_at, proxy = @proxy, cf_clearance = @cf_clearance, user_agent = @user_agent, token = @token
        WHERE id = @id
    `).run({
        id: result.jobData.id,
        status: 1, // success
        finished_at: new Date().toISOString(),
        proxy: result.data.proxy,
        cf_clearance: result.data.cfClearance,
        user_agent: result.data.userAgent,
        token: result.data.turnstileCheck
    });
}
